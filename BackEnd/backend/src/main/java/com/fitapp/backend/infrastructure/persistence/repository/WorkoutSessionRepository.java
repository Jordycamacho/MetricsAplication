...
public interface WorkoutSessionRepository extends JpaRepository<WorkoutSessionEntity, Long>, 
     ...
     */
    @EntityGraph(attributePaths = {"routine", "exercises", "exercises.exercise", "exercises.sets", "exercises.sets.parameters"})
    @Query("SELECT ws FROM WorkoutSessionEntity ws WHERE ws.id = :id AND ws.routine.user.id = :userId")
     ...
     */
    @Query("SELECT ws FROM WorkoutSessionEntity ws WHERE ws.routine.user.id = :userId ORDER BY ws.startTime DESC")
     ...
     */
    @Query("SELECT COALESCE(SUM(ws.totalVolume), 0) FROM WorkoutSessionEntity ws WHERE ws.routine.user.id = :userId")
