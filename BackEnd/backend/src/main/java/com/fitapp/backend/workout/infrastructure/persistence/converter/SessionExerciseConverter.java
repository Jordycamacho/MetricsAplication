...
public class SessionExerciseConverter {
    ...
    public SessionExerciseModel toDomain(SessionExerciseEntity entity) {
        if (entity == null) {
            log.warn("SESSION_EXERCISE_CONVERTER_NULL_ENTITY");
            return null;
        }

        log.debug("SESSION_EXERCISE_CONVERTER_TO_DOMAIN | id={} | exerciseId={}",
    ...
    public SessionExerciseEntity toEntity(SessionExerciseModel model,
            com.fitapp.backend.workout.infrastructure.persistence.entity.WorkoutSessionEntity session) {
        if (model == null) {
            log.warn("SESSION_EXERCISE_CONVERTER_NULL_MODEL");
            return null;
        }

        log.debug("SESSION_EXERCISE_CONVERTER_TO_ENTITY | id={} | exerciseId={}",
    ...
    /**
     * Convierte model a response.
     *
     * <p>FIX: la versión anterior hacía un findById extra aquí para obtener el nombre
     * del ejercicio, causando un query N+1. Ahora el exerciseName no se incluye
     * porque el model no lo lleva — usa {@link #toResponseFromEntity} cuando tengas
     * la entidad disponible, o carga el nombre antes de construir el model.
     */
    public SessionExerciseResponse toResponse(SessionExerciseModel model) {
        if (model == null) {
            log.warn("SESSION_EXERCISE_CONVERTER_NULL_MODEL_TO_RESPONSE");
            return null;
        }

        log.debug("SESSION_EXERCISE_CONVERTER_TO_RESPONSE | id={}", model.getId());

        return SessionExerciseResponse.builder()
                .id(model.getId())
                .exerciseId(model.getExerciseId())
                .exerciseName(null) // populated by toResponseFromEntity when entity is available
    ...
    // ── Entity → Response DTO (preferred: avoids extra DB queries) ────────────

    public SessionExerciseResponse toResponseFromEntity(SessionExerciseEntity entity) {
        if (entity == null) {
            log.warn("SESSION_EXERCISE_CONVERTER_NULL_ENTITY_TO_RESPONSE");
            return null;
        }

        log.debug("SESSION_EXERCISE_CONVERTER_TO_RESPONSE_FROM_ENTITY | id={}", entity.getId());
