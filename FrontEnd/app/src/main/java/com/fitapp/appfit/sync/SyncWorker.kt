...
import com.fitapp.appfit.database.entities.PendingSyncOperation
...
/**
 * SyncWorker — se ejecuta cuando hay conexión y hay operaciones pendientes.
 *
 * Procesa en orden:
 *  1. Cola de PendingSyncOperation (rutinas, ejercicios, etc.) — FIFO
 *  2. Sesiones de workout pendientes de enviar al back
 *
 * Si una operación falla 3 veces, la deja para la próxima sincronización.
 */
class SyncWorker(
    context: Context,
    ...
    // Reutiliza WorkoutRepository para no duplicar la lógica de sync de workouts
    private val workoutRepository = WorkoutRepository(context)

    override suspend fun doWork(): Result {
        if (!networkChecker.isOnline()) {
            Timber.d("SyncWorker: sin conexión, reintentando más tarde")
            return Result.retry()
        }

        Timber.i("SyncWorker: iniciando sincronización")

        // ── 1. Cola general (rutinas, ejercicios) ─────────────────────────────
        val routineSuccess = processRoutineQueue()

        // ── 2. Workouts pendientes ────────────────────────────────────────────
        val workoutSynced = workoutRepository.syncAllPendingSessions()
        Timber.i("SyncWorker: $workoutSynced sesiones de workout sincronizadas")
    ...
    private suspend fun processRoutineQueue(): Boolean {
        ...
        if (pending.isEmpty()) {
            Timber.i("SyncWorker: nada en la cola de rutinas")
            return true
        }
        ...
        for (operation in pending) {
            if (operation.retryCount >= 3) {
                Timber.w("SyncWorker: operación ${operation.operationId} superó reintentos, saltando")
                continue
            }
        ...
        return try {
            when (op.entityType) {
                ...
                "ROUTINE_EXERCISE" -> processRoutineExerciseOperation(op)
                    ...
                    Timber.w("Tipo de entidad desconocido: ${op.entityType}")
                    syncDao.delete(op)
                    true
                }
            }
        } catch (e: Exception) {
            Timber.e(e, "Error procesando operación ${op.operationId}")
            false
        }
    }
        ...
        return when (op.operation) {
            "CREATE" -> {
                ...
                if (response.isSuccessful) {
                    ...
                    syncDao.delete(op)
                    Timber.i("Rutina creada en servidor: $serverId")
                    true
                } else {
                    Timber.w("Error creando rutina: ${response.code()}")
                    false
                }
            }
            ...
            "UPDATE" -> {
                ...
                if (response.isSuccessful) {
                    ...
                    syncDao.delete(op)
                    Timber.i("Rutina actualizada en servidor: ${op.entityId}")
                    true
                } else {
                    Timber.w("Error actualizando rutina: ${response.code()}")
                    false
                }
            }
            ...
            "DELETE" -> {
                ...
                if (response.isSuccessful || response.code() == 404) {
                    ...
                    syncDao.delete(op)
                    Timber.i("Rutina eliminada en servidor: ${op.entityId}")
                    true
                } else {
                    Timber.w("Error eliminando rutina: ${response.code()}")
                    false
                }
            }
    ...
    private suspend fun processRoutineExerciseOperation(op: PendingSyncOperation): Boolean {
        // Implementar igual que routines siguiendo el mismo patrón
        syncDao.delete(op)
        return true
    }
    ...
    companion object {
        ...
        fun syncNow(context: Context) {
            ...
            Timber.i("Sincronización inmediata programada")
        }
    }
}