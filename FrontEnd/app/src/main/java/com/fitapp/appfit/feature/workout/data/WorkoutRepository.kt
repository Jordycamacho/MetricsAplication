...
class WorkoutRepository(private val context: Context) {
    ...
    suspend fun getLastValuesForRoutine(routineId: Long): Resource<Map<Long, LastExerciseValuesResponse>> {
        ...
        return try {
            ...
            if (response.isSuccessful) {
                ...
                if (body != null) {
                    // Backend returns Map<String, ...> because JSON keys are always strings
    ...
    suspend fun getTotalVolume(): Resource<Double> = call { service.getTotalVolume() }
    ...
    private fun buildSetResults(
        ...
        completedSetIds.forEach { setTemplateId ->
            ...
            if (setData == null) {
                Log.w(TAG, "SKIP_SET_NO_DATA | setId=$setTemplateId")
                return@forEach
            }
    ...
    private fun buildSaveWorkoutSessionRequest(
        ...
        // Group by (setTemplateId, exerciseId) — one entry per set
        val groupedBySet = results.groupBy { it.setTemplateId to it.exerciseId }

        // Group by exerciseId to compute per-exercise position
        val groupedByExercise = results.groupBy { it.exerciseId }
        // Build an ordered list of setTemplateIds per exercise (preserving insertion order)
        ...
        val setExecutions = groupedBySet.entries.map { (key, params) ->
            val (setTemplateId, exerciseId) = key

            // FIX: position is per-exercise (1-based index within the exercise)
            ...
            val validParams = params.mapNotNull { result ->
                ...
                if (!hasValue) {
                    Log.w(TAG, "SKIP_EMPTY_PARAM | parameterId=${result.parameterId}")
